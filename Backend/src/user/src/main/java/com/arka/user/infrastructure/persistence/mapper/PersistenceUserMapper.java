...
public class PersistenceUserMapper implements IPersistanceMapper< User, UserTable >{
    ...
    public User toDomain(UserTable c) {
        return User.builder().id(c.getId())
            .name(UserName.create(c.getFirstName(), c.getLastName(), c.getFirstSurname(), c.getLastSurname()))
            .email(c.getEmail())
    ...
    public UserTable toEntity(User c) {
        return UserTable.builder().id(c.getId())
            .firstName(c.getName().getFirstName())
            .lastName(c.getName().getLastName())
            .firstSurname(c.getName().getFirstSurname())
            .lastSurname(c.getName().getLastSurname())
            .email(c.getEmail())
            .username(c.getUsername())
            .password(c.getPassword())
            .isActive(c.isActive())
